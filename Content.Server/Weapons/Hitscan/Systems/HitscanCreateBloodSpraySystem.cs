...
public sealed class HitscanCreateBloodSpraySystem : EntitySystem
{
    ...
    private void OnHitscanHit(Entity<HitscanCreateBloodSprayComponent> ent, ref HitscanDamageDealtEvent args)
    {
        ...
        var distance = Math.Abs((Transform(args.Target).Coordinates.Position - Transform(args.Data.Gun).Coordinates.Position).Length());
        var hitEntityCords = Transform(args.Target).Coordinates;
